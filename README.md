# GD Trade

GD Trade 是一个面向 A 股持仓监控与决策辅助的 Android 原生 App。当前 V1 只做持仓管理、观察池、风险提示、交易记录和人工确认入口，不实现自动证券交易。

## V1 边界

- 不自动提交真实证券买卖订单。
- 不存储证券账户密码、短信验证码、Cookie 或任何交易登录凭据。
- 行情层使用 `StaticMarketRepository` 静态样例数据，不声称具备真实实时行情能力。
- 风险引擎可以否决买入信号，UI 只展示提示和人工确认入口。
- “打开同花顺”只用于跳转人工确认，不代表下单。

## 当前功能

- 首页策略驾驶舱。
- 当前持仓：
  - 华天科技 002185：200 股
  - 华夏中证人工智能主题 ETF 515070：800 份
  - 京东方 A 000725：100 股
- 动态观察池与信号状态展示。
- 交易记录：
  - 2026-07-13 华天科技 24.62 元卖出 100 股
  - 2026-07-13 京东方 A 6.92 元卖出 200 股
- 一键打开同花顺进行人工确认。
- 本地数据维护：新增/删除持仓、观察股票和交易记录，支持恢复样例数据。
- 行情接口：尝试读取腾讯行情接口，显示价格、涨跌幅和接口时间；失败时回退本地静态样例。
- GPT研究接口：通过后端代理生成研究辅助意见，手机端不保存 OpenAI API Key。


## 本地数据维护

App 会把用户维护的数据保存在手机本地应用私有存储中：

- 新增持仓会覆盖同代码的旧持仓。
- 新增观察股票会覆盖同代码的旧观察项。
- 新增交易记录会追加到记录列表顶部。
- 删除按钮只删除当前条目。
- “恢复样例”会把持仓、观察池和交易记录恢复为 V1 样例数据。
- 卸载 App 会清除本地保存的数据。

当前 V1 仍不连接证券账户，也不保存任何证券登录凭据。

## 行情接口

V1 会尝试通过腾讯行情接口读取持仓股票的参考价格、涨跌幅和接口时间。该数据可能存在延迟或不可用，App 会在界面中标注来源；接口失败时回退本地静态样例数据。

行情数据仅用于持仓监控与人工决策辅助，不构成交易建议，也不会触发自动下单。
## 架构

项目采用分层结构：

- `domain/model`：持仓、候选股票、行情、交易记录、账户目标等领域模型。
- `domain/risk`：风险引擎与风险决策。
- `data/repository`：仓储接口与 V1 静态实现。
- `ui/dashboard`：策略驾驶舱界面、UI 状态与 ViewModel。
- `ui/theme`：Compose 主题。

UI 状态由 `DashboardViewModel` 使用 `StateFlow<DashboardUiState>` 管理。仓储层通过接口抽象，后续可替换为真实行情或本地数据库实现。

## 信号状态

V1 支持以下信号状态：

- 持有观察
- 减仓风险提示
- 暂不追高
- 等待回调
- 出现研究型买点
- 高位不追
- 仅观察

## 本地运行

推荐使用 Android Studio 打开项目并同步 Gradle。

当前仓库没有提交 Gradle Wrapper。本地如需命令行构建，应先在可信环境中生成 Gradle Wrapper，再运行：

```powershell
.\gradlew test
.\gradlew assembleDebug
```

远端 PR 已配置 GitHub Actions，会使用 Java 17 和 Gradle 8.10.2 运行：

```powershell
gradle test --no-daemon
gradle assembleDebug --no-daemon
```


## GPT研究接口

App 不会在手机端保存 OpenAI API Key。若要启用 GPT 研究意见，需要配置一个后端代理地址，由后端安全调用 OpenAI API。

本地构建时可在 `gradle.properties` 或命令行中配置：

```properties
gptAdvisorEndpoint=https://你的后端域名/gpt-advisor
```

App 会向该地址发送 POST 请求，请求体为：

```json
{"prompt":"包含持仓、行情、观察池和交易记录的中文研究提示"}
```

后端返回纯文本或 JSON 文本均可，App 会直接展示返回内容。GPT 输出仅作为研究辅助，不提供确定性买卖指令，不承诺收益，不自动交易。
## 下一步建议

1. 加入 Gradle Wrapper，固定命令行构建入口。
2. 增加本地持久化仓储，例如 Room。
3. 接入真实行情前先定义数据来源、刷新频率、延迟标识和失败兜底。
4. 扩展风险引擎规则，并为每条规则补充单元测试。
