...
UI 状态由 `DashboardViewModel` 通过 `StateFlow<DashboardUiState>` 对外暴露。当前运行时由 `DashboardViewModel.Factory` 创建一个 `LocalPreferenceRepository`，同时作为 `PortfolioRepository` 和 `MarketRepository` 使用。
...
- `LocalPreferenceRepository`：当前主仓储实现；使用 SharedPreferences 保存本地数据；尝试读取腾讯行情接口；接口失败时回退样例数据。
...
当前 App 默认使用 `LocalPreferenceRepository`：

- 持仓数据：手机本地 SharedPreferences，首次启动使用默认样例。
- 观察池数据：手机本地 SharedPreferences，首次启动使用默认样例。
- 交易记录：手机本地 SharedPreferences，首次启动使用默认样例。
- 行情数据：点击刷新或状态组合更新时，尝试访问腾讯行情接口 `https://qt.gtimg.cn/q=...`。
- 行情失败兜底：腾讯接口失败、解析失败或无数据时，回退 `LocalPreferenceRepository.defaultQuotes` 或生成“行情接口暂不可用，未使用实时行情”的空行情提示。
...
- `LocalPreferenceRepository.defaultPositions`：默认持仓。
- `LocalPreferenceRepository.defaultTradeRecords`：默认交易记录。
- `LocalPreferenceRepository.defaultCandidates`：默认观察池。
- `LocalPreferenceRepository.defaultQuotes`：默认静态行情兜底。
...
- Room 数据库：当前本地持久化仍是 SharedPreferences 字符串序列化，不适合长期复杂数据。
...
- SharedPreferences 序列化使用分隔符和 URL 编码，短期可用，但不适合复杂字段、迁移和查询。
...
1. 引入 Room 数据库，建立 `PositionEntity`、`StockCandidateEntity`、`TradeRecordEntity` 和迁移策略。
...
8. 若下一步做 Room，建议先保留 `PortfolioRepository` 和 `MarketRepository` 接口不变，只替换实现，降低 UI 层改动范围。